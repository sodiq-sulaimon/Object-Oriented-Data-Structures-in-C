...
Name: Sodiq Sulaimon
Email: sodiq.sulaimon204@gmail.com
...
PNG illinify(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++){
    for (unsigned y = 0; y < image.height(); y++){
      ...
      int maxHue = 360;
      int orangeDistance = abs(pixel.h - orangeHue);
      int blueDistance = abs(pixel.h - blueHue);
      ...
      pixel.h = (orangeDistance < blueDistance) ? orangeHue : blueHue; 
      }
    }
    return image;
  }
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for (unsigned x = 0; x < firstImage.width(); x++){
    for (unsigned y = 0; y < firstImage.height(); y++){
      ...
      if (x < secondImage.width() && y < secondImage.width()){
